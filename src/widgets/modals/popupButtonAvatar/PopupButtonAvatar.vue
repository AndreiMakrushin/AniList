...
<template>
  <div class="absolute right-2 shadow-shadowDrop rounded-b-[10px] text-[14px] p-3">
    <div class="flex flex-col justify-center items-center gap-3">
      <div
      @click="animeStore.modal = true"
        class="text-green-500 w-[100%] flex flex-row items-center justify-start gap-2 text-center cursor-pointer hover:text-[#413d3d] duration-short"
      >
        <svg
          width="20"
          height="20"
        ...
        <Span :text="'Загрузить фотографию'" />
      </div>
      <div
        @click="deleteAvatar"
        class="text-green-500 w-[100%] flex flex-row items-center justify-start gap-2 text-center cursor-pointer hover:text-[#413d3d] duration-short"
